  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        await sql`
          INSERT INTO drizzle.__drizzle_migrations (hash, created_at)
          VALUES (${recentHistoryHash}, ${recentHistoryAppliedAt}),
                 (${historicalUnknownHash}, ${1781166037000})
        `;
        const [recentIndex] = await sql`SELECT 'heartbeat_runs_company_agent_created_idx'::regclass::oid::int AS oid`;
        recentHistoryIndexOid = recentIndex.oid;
        ...
        await sql`
          ...
          VALUES (${agentId}, ${companyId}, 'Legacy migration agent', 'codex_subscription_2_local',
            '{"model":"gpt-5.6-terra","modelReasoningEffort":"xhigh","codexAuthSourceHome":"/fixture/subscription-2"}'::jsonb,
            '{"heartbeat":{"enabled":true},"modelProfiles":{"cheap":{"model":"retired-model"}}}'::jsonb)
        `;
        await sql`
      ...
      const verifySql = postgres(connectionString, { max: 1, onnotice: () => {} });
      try {
        expect(await verifySql`SELECT 'heartbeat_runs_company_agent_created_idx'::regclass::oid::int AS oid`)
          .toEqual([{ oid: recentHistoryIndexOid }]);
        const history = await verifySql`
          SELECT hash, created_at FROM drizzle.__drizzle_migrations
          WHERE hash IN (${recentHistoryHash}, ${historicalUnknownHash}) ORDER BY created_at
        `;
        expect(history.map((row) => ({ hash: row.hash, createdAt: Number(row.created_at) }))).toEqual([
          { hash: historicalUnknownHash, createdAt: 1781166037000 },
          { hash: recentHistoryHash, createdAt: recentHistoryAppliedAt },
        ]);
        expect(await verifySql`SELECT adapter_type, adapter_config, runtime_config FROM agents WHERE id = ${agentId}`)
          .toEqual([{
            adapter_type: "codex_subscription_2_local",
            adapter_config: { model: "gpt-5.6-terra", modelReasoningEffort: "xhigh", codexAuthSourceHome: "/fixture/subscription-2" },
            runtime_config: { heartbeat: { enabled: true } },
          }]);
        expect((await inspectMigrations(connectionString)).status).toBe("upToDate");
