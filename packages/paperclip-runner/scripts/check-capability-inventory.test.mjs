...
test("fork skill headings may change without changing the external eval baseline", () => {
  const inventories = validInventories();
  inventories.capabilities.rows.push(row("fork-operator-handoff"));
  assert.deepEqual(validateInventorySchema(inventories, inventorySchema), []);
  assert.deepEqual(validateInventories(inventories), []);
});

test("skills-only regeneration checks local sources without the private eval corpus", () => {
  const output = execFileSync(process.execPath, [
    resolve(import.meta.dirname, "generate-capability-inventory.mjs"), "--skills-only", "--check",
  ], {
    encoding: "utf8",
    env: { ...process.env, PAPERCLIP_EVALS_ROOT: "/unavailable-paperclip-evals-fixture" },
  });
  assert.match(output, /match the canonical baselines/);
});
