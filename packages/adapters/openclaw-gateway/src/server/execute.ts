...
function buildWakeText(
  ...
  const lines = [
    ...
    "   - For plan approval, update the plan document first, then create request_confirmation targeting the latest plan revision with idempotencyKey confirmation:{issueId}:plan:{revisionId}; wait for acceptance before materializing implementation subtasks.",
    "   - If woken after an accepted plan confirmation, check GET /api/issues/{issueId}/accepted-plan-decompositions, then create approved children through POST /api/issues/{issueId}/accepted-plan-decompositions using the accepted plan revision id. For sprint backlog batches, set child status explicitly to backlog; use todo only when execution should start now.",
    "   - If woken with wakeReason issue_interaction_stale_target or workspaceRefreshReason stale_request_confirmation_repair, refresh the issue, latest plan document, and interactions; do not continue implementation. If approval is still needed, create one fresh request_confirmation targeting the latest plan revision.",
