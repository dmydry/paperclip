...
function parseCommandExecutionItem(
  ...
  const output = truncateLargeOutput(
    redactHomePathUserSegments(redactSecrets(asString(item.aggregated_output)).replace(/\s+$/, "")),
  );
  ...
  if (phase === "started") {
    return [{
      ...
      toolUseId: id || safeCommand || "command_execution",
      input: {
        id,
        command: safeCommand,
      },
