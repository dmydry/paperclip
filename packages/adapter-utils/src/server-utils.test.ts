...
describe("runChildProcess", () => {
  ...
  it("treats an empty env override as an explicit empty value", async () => {
    ...
    const result = await runChildProcess(
      ...
      [
        "-e",
        "process.stdout.write(Object.prototype.hasOwnProperty.call(process.env, 'OPENAI_API_KEY') ? `present:${process.env.OPENAI_API_KEY}` : 'absent');",
      ],
      {
        cwd: process.cwd(),
        env: { OPENAI_API_KEY: "" },
...
describe("renderPaperclipWakePrompt", () => {
  ...
  it("keeps the default local-agent prompt action-oriented", () => {
    ...
    expect(DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE).toContain(
      "After a deterministic 400/401/403/404/409/422 control-plane rejection",
    );
    expect(DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE).toContain(
    ...
    expect(DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE).toContain("Wait for acceptance before materializing implementation subtasks");
    expect(DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE).toContain("accepted-plan-decompositions");
    expect(DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE).toContain("set child status explicitly to backlog");
    expect(DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE).toContain(
  ...
  it("adds the execution contract to resume delta prompts and opted-in fresh prompts", () => {
    ...
    for (const prompt of [
      ...
      expect(prompt).toContain(
        "If no valid disposition is recorded, attempt one valid disposition",
      );
      expect(prompt).toContain(
        "After a deterministic 400/401/403/404/409/422 control-plane rejection",
      );
      ...
      expect(prompt).not.toContain("named unblock owner/action");
    }
  });
