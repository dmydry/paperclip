...
export const DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE = [
  ...
  "- Final disposition checklist: mark `done` when complete; use `in_review` only with a real reviewer, approval, interaction, or monitor path; use `blocked` only with unresolved first-class blockers; create delegated follow-up issues with blockers when another agent owns the next step; keep `in_progress` only when a live continuation path exists.",
  "- Prefer the smallest verification that proves the change; do not default to full workspace typecheck/build/test on every heartbeat unless the task scope warrants it.",
  "- After a deterministic 400/401/403/404/409/422 control-plane rejection, do not retry that write or probe alternate endpoints/tokens. For a network failure, 429, or 5xx, read current state before at most one retry; never duplicate an uncertain commit. Continue independent useful work and report any unresolved failure once, and rely on the adapter/runtime status channel as the sanctioned fallback.",
  ...
  "- For plan approval, update the plan document first, then create request_confirmation targeting the latest plan revision with idempotencyKey confirmation:{issueId}:plan:{revisionId}. Wait for acceptance before materializing implementation subtasks, and create a fresh confirmation after superseding board/user comments if approval is still needed.",
  "- If woken after an accepted plan confirmation, do not close the source issue as merely approved. Check GET /api/issues/$PAPERCLIP_TASK_ID/accepted-plan-decompositions, then create the approved children through POST /api/issues/$PAPERCLIP_TASK_ID/accepted-plan-decompositions using the accepted plan revision id. For sprint backlog batches, set child status explicitly to backlog; use todo only when execution should start now.",
  "- If woken with wakeReason issue_interaction_stale_target or workspaceRefreshReason stale_request_confirmation_repair, refresh the issue, latest plan document, and interactions; do not continue implementation. If approval is still needed, create one fresh request_confirmation bound to the latest plan revision with idempotencyKey confirmation:{issueId}:plan:{revisionId}; otherwise leave a concise blocker/comment explaining why no fresh approval is valid.",
  ...
  const executionContractLines = externalChatContract
    ...
    : recoveryScoped
      ...
      : includeExecutionContract
        ? [
            "Execution contract: take concrete action in this heartbeat when the issue is actionable; do not stop at a plan unless planning was requested. Leave durable progress and then give the issue a clear final disposition before ending the heartbeat: `done`, `in_review` with a real reviewer/approval/interaction path, `blocked` with unresolved first-class blockers, delegated follow-up issues with blockers, or `in_progress` only when a live continuation path exists. Immediately before returning, verify that Paperclip records one of those dispositions; a successful process exit or final response is not sufficient. If no valid disposition is recorded, attempt one valid disposition. If rejected, preserve the last valid state and stop; never invent an approval or alternate mutation to satisfy liveness. After a deterministic 400/401/403/404/409/422 control-plane rejection, stop retrying it for the rest of the heartbeat, continue useful work, report the failure in the final response, and rely on the adapter/runtime status channel as the sanctioned fallback. Use child issues for long or parallel delegated work instead of polling. Comments, documents, screenshots, work products, and `Remaining` bullets are evidence, not valid liveness paths by themselves.",
            "",
          ]
        : [];
