...
const INBOX_UNREAD_ISSUE_STATUS_FILTER = "backlog,todo,in_progress,in_review,blocked";
...
function StreamlinedInbox() {
  ...
  const {
    ...
    queryKey: [...queryKeys.issues.listUnreadTouchedByMe(selectedCompanyId!), "compact", "with-routine-executions", "live-descendant-summary", INBOX_ISSUE_LIST_LIMIT],
    queryFn: () =>
      issuesApi.listCompact(selectedCompanyId!, {
        unreadForUserId: "me",
        status: INBOX_UNREAD_ISSUE_STATUS_FILTER,
        includeRoutineExecutions: true,
        includeLiveDescendantSummary: true,
        limit: INBOX_ISSUE_LIST_LIMIT,
      }).then((rows) => rows as Issue[]),
  ...
  const unreadTouchedIssues = useMemo(
    () =>
      applyIssueFilters(
        getRecentTouchedIssues(unreadTouchedIssuesRaw.filter((issue) => issue.isUnreadForMe)),
        issueFilters,
        currentUserId,
        true,
        liveIssueIds,
    ...
    [unreadTouchedIssuesRaw, issueFilters, currentUserId, liveIssueIds, issueFilterContext],
  );
