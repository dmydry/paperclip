  ...
  useEffect(() => {
    ...
    void Promise.all(
      missingDocs.map(async (doc) => {
        ...
        try {
          const fullDoc = await documentSubject.getDocument(doc.key);
      ...
      <div className="space-y-3">
        {sortedDocuments.map((doc) => {
          ...
          const displayedTitle = selectedHistoricalRevision
            ...
            : activeDraft?.title ?? currentRevision.title ?? doc.title ?? "";
          ...
          const displayedRevisionNumber =
            selectedHistoricalRevision?.revisionNumber ?? currentRevision.revisionNumber ?? doc.latestRevisionNumber;
          const displayedUpdatedAt =
            selectedHistoricalRevision?.createdAt ?? currentRevision.createdAt ?? doc.updatedAt;
