...
export function NewIssueDialog() {
  ...
  const activeUserAssigneeOptions = useMemo<InlineEntityOption[]>(() => {
    const options: InlineEntityOption[] = [];
    const seen = new Set<string>();
    for (const option of currentUserAssigneeOption(currentUserId)) {
      seen.add(option.id);
      options.push(option);
    }
    for (const member of activeUserMembers) {
      const id = assigneeValueFromSelection({ assigneeUserId: member.principalId });
    ...
    return options;
  }, [activeUserMembers, currentUserId]);
  const assigneeOptions = useMemo<InlineEntityOption[]>(
    () => [
      ...activeUserAssigneeOptions,
      ...sortAgentsByRecency(
    ...
    [activeUserAssigneeOptions, agents, recentAssigneeIds],
  );
