...
export function IssueFiltersPopover({
  ...
  assigneeUsers?: AssigneeUserOption[];
}) {
              ...
              <div className="space-y-1">
                ...
                <div className="max-h-32 space-y-0.5 overflow-y-auto">
                  ...
                  {(assigneeUsers ?? []).map((user) => (
                    <label key={user.id} className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      <Checkbox
                        checked={state.assignees.includes(user.id)}
                        onCheckedChange={() => onChange({ assignees: toggleIssueFilterValue(state.assignees, user.id) })}
                      />
                      <User className="h-3.5 w-3.5 text-muted-foreground" />
