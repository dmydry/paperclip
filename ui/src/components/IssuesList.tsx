...
export function IssuesList({
  ...
  const activeUserMembers = useMemo(
    () =>
      (companyMembers?.users ?? []).filter(
        (member) => member.status === "active" && member.principalId !== "local-board",
      ),
    [companyMembers?.users],
  );
  ...
  }, [filtered, shouldCapRenderedRows, viewState.viewMode]);

  const hasMoreRenderedRows = shouldCapRenderedRows && renderedIssueRowLimit < filtered.length;
      ...
      <div className="flex items-center justify-between gap-2 sm:gap-3">
        ...
        <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
          ...
          <IssueFiltersPopover
            ...
            agents={agents}
            assigneeUsers={assigneeUserOptions}
