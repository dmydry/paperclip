...
describeEmbeddedPostgres("issueService.list participantAgentId", () => {
  ...
  it("excludes plugin operation issues from unread inbox counts", async () => {
    ...
    // Our inbox policy requires a real read/comment/assignment, not bare creation.
    await svc.markRead(companyId, normalIssueId, userId, new Date("2026-01-01T00:00:00Z"));
    await svc.markRead(companyId, operationIssueId, userId, new Date("2026-01-01T00:00:00Z"));
    await db.insert(issueComments).values([
      {
        companyId,
...
describeEmbeddedPostgres("issueService.create workspace inheritance", () => {
  ...
  it("createChild uses the target project workspace instead of inheriting parent workspace for cross-project children", async () => {
    const companyId = randomUUID();
