...
describe("allocateRuntimeServicePort", () => {
  it("skips the dedicated managed HTTPS app and HMR ranges", async () => {
    const candidates = [42_000, 52_000, 45_439];

    await expect(allocateRuntimeServicePort({
      probe: async () => candidates.shift()!,
      portOwnerLookup: async () => null,
    })).resolves.toBe(45_439);
  });
});
...
describe("realizeExecutionWorkspace", () => {
  ...
  it("refreshes a clean unstarted reused worktree to the latest remote base ref", async () => {
    const { sourceRoot, repoRoot } = await createTrackedTempRepo();
    ...
    expect(second.created).toBe(false);
    expect(second.cwd).toBe(first.cwd);
    expect(await runGitStdout(second.cwd, ["rev-parse", "HEAD"])).toBe(freshRemoteMain);
  }, 20_000);
  ...
  it("records teardown and cleanup operations when a recorder is provided", async () => {
    ...
    expect(operations[1]?.metadata).toMatchObject({
      ...
      instanceRoot: path.join(await fs.realpath(worktreesDir), "instances", instanceId),
    });
...
describeEmbeddedPostgres("workspace runtime service control persistence", () => {
  ...
  it("persists provisioning before starting and excludes provision time from readiness timeout", async () => {
    ...
    try {
      ...
      expect(runningRow.port).toEqual(expect.any(Number));
      await expect(fetch(services[0]!.url!)).resolves.toMatchObject({ ok: true });
