...
describe("codex execute", () => {
  ...
  it("uses subscription auth while an empty OPENAI_API_KEY blocks host key inheritance", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-codex-empty-key-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
...
`;
  ...
  it("isolates connector skills by agent and revision without changing the selected model identity", async () => {
    ...
    try {
      ...
      expect(first.codexHome).toContain(path.join("agents", "agent-1", "codex-home", "connector-runtimes"));
      ...
      expect(removed.codexHome).toContain(path.join("agents", "agent-1", "codex-home"));
      expect(await fs.realpath(path.join(removed.codexHome!, "auth.json"))).toBe(await fs.realpath(path.join(sourceHome, "auth.json")));
