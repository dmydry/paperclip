...
describe("agent issue mutation checkout ownership", () => {
  ...
  it("allows peer agents to leave plain comments on another agent's active checkout", async () => {
    const res = await request(await createApp(peerActor()))
      .post(`/api/issues/${issueId}/comments`)
      .send({ body: "Please recheck the QA failure before closing." });
    ...
    expect(mockIssueService.addComment).toHaveBeenCalledWith(
      issueId,
      "Please recheck the QA failure before closing.",
