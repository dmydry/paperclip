...
describe("agent issue mutation checkout ownership", () => {
  ...
  it.each(["in_progress", "in_review"])("wakes the assignee once to repair stale plan confirmations after document updates (%s)", async (status) => {
    mockIssueService.getById.mockResolvedValue(makeIssue({ status }));
    mockIssueService.listReviewAttention.mockResolvedValue(new Map([[issueId, {
      state: "stalled", paths: [], reason: "The old plan target expired",
    }]]));
    const app = await createApp(boardActor());
