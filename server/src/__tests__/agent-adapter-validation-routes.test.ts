...
describe("agent routes adapter validation", () => {
  ...
  it("adds isolated CODEX_HOME without OPENAI_API_KEY when creating codex_local agents", async () => {
    const app = await createApp();
    const res = await requestApp(app, (baseUrl) =>
      request(baseUrl)
        .post("/api/companies/company-1/agents")
  ...
  it("adds isolated CODEX_HOME without OPENAI_API_KEY when updating codex_local agents", async () => {
    const app = await createApp();
    const res = await requestApp(app, (baseUrl) =>
      request(baseUrl)
