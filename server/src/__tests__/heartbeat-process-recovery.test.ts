...
describeEmbeddedPostgres("heartbeat orphaned process recovery", () => {
  ...
  it("escalates an exhausted successful handoff to board without another automatic recovery wake", async () => {
    const { companyId, agentId, runId, issueId } =
      await seedStrandedIssueFixture({
        status: "in_progress",
        runStatus: "succeeded",
    ...
    expect(recoveryAction).toMatchObject({ ownerType: "board", ownerAgentId: null });
    expect(firstRecoveryWakes).toHaveLength(0);
    ...
    expect(repeatedRecoveryWakes).toHaveLength(0);
