...
describeEmbeddedPostgres("low-trust red-team HTTP route regression suite", () => {
  ...
  it("preserves direct-parent reporting while containing run-scoped standard-trust comments", async () => {
    const fixture = await seedLowTrustFixture(db);
    ...
    const forbiddenRunScopedComments = [
      request(standardApp)
    ...
    for (const forbiddenRunScopedComment of forbiddenRunScopedComments) {
      const response = await forbiddenRunScopedComment;
      expect(response.status, JSON.stringify(response.body)).toBe(403);
      expect(response.body.details.code).toBe("issue_write_not_visible");
    }
