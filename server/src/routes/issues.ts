...
async function queueResolvedInteractionContinuationWakeup(input: {
  ...
  if (
    ...
    return;
  if (input.interaction.status === "expired" && !reviewPathLost && !staleTargetRepair) return;
  ...
  const wakeIdempotencyKey = input.idempotencyKey ?? (staleTargetRepair
...
export function issueRoutes(
  ...
  async function logExpiredRequestConfirmations(input: {
    ...
    for (const interaction of input.interactions) {
      ...
      // Ordinary expiry already has one aggregated recovery/comment wake below.
      // Only our stale-target repair needs its own continuation here; upstream's
      // broader reviewPathLost continuation would otherwise enqueue both paths.
      if (!isStaleTargetRequestConfirmation(interaction)) continue;
  ...
  async function queueExpiredInteractionReviewPathRecovery(input: {
    ...
    const recoveryInteractions = input.interactions.filter((interaction) => !isStaleTargetRequestConfirmation(interaction));
    if (
      recoveryInteractions.length === 0 ||
      input.issue.status !== "in_review" ||
    ...
    const interactionIds = [
      ...new Set(recoveryInteractions.map((interaction) => interaction.id)),
    ].sort();
  ...
  router.patch(
    ...
    async (req, res) => {
      ...
      const returnedToTodoViaComment = shouldWakeAssigneeOnCommentBackToTodo({
        previousStatus: existing.status, nextStatus: issue.status, commentBody, assigneeAgentId: issue.assigneeAgentId,
      });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      let cancelledScheduledRetryRunId: string | null = null;
      if (
        autoReturnToTodo || (effectiveMoveToTodoRequested &&
        (isClosed ||
          (isBlocked && !hasUnresolvedFirstClassBlockers) ||
          shouldResumeInProgressScheduledRetry))
      ) {
        ...
        reopened = autoReturnToTodo || isClosed || (isBlocked && !hasUnresolvedFirstClassBlockers);
