...
function isMcpGatewayProtocolPath(path: string) {
  const internalGatewayId = /^\/api\/tool-gateway\/gateways\/([^/]+)\/mcp\/?$/.exec(path)?.[1];
  return publicMcpGatewayProtocolPath.test(path)
    || (internalGatewayId !== undefined && isUuidLike(internalGatewayId));
}
...
export function actorMiddleware(db: Db, opts: ActorMiddlewareOptions): RequestHandler {
  ...
  return async (req, _res, next) => {
    ...
    // the protocol route can verify its run-scoped credential. The internal MCP
    // alias uses the same verifier. Other /api routes retain normal actor auth.
