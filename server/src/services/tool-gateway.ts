...
const ACTION_REQUEST_EXECUTION_WAIT_MS = DEFAULT_APPROVED_EXECUTION_TIMEOUT_MS + 5_000;
...
export function createToolGatewayService(
  ...
  async function connectedRemoteApprovalContext(
    ...
    if (tool.providerType === "paperclip_slack_chat") {
      ...
      return {
        snapshot: { endpointId: authority.endpoint.id, userId: authority.userId, revision: authority.revision, identityContextId: authority.identityContextId },
  ...
  async function waitForActionRequestExecution(actionRequestId: string) {
    ...
    let executionWaitMs = ACTION_REQUEST_EXECUTION_WAIT_MS;
    while (true) {
