...
function recoveryActionDrivenRunSkipReason(run: HeartbeatRunRow) {
  const context = readRecord(run.contextSnapshot);
  if (
    readString(context.wakeReason) === "source_scoped_recovery_action" ||
    readString(context.recoveryActionId) !== null
  ) {
    return "recovery action run owns its own follow-up path";
  }
  if (readString(context.source) === "issue_recovery_action") {
    return "source-scoped recovery run owns its own recovery path";
  }
  return null;
}
...
export function decideSuccessfulRunHandoff(input: {
  ...
  const recoveryActionSkipReason = recoveryActionDrivenRunSkipReason(run);
  if (recoveryActionSkipReason) return { kind: "skip", reason: recoveryActionSkipReason };
