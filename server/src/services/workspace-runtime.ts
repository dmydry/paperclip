...
export async function getGitWorktreeStatus(worktreePath: string): Promise<{
  ...
  const stdout = await runExpensiveGitStatus({
    args: ["status", "--porcelain", "--untracked-files=all"],
    cwd: worktreePath,
    operation: "workspace_runtime.external_worktree_status",
    fairnessKeys: [`worktree:${path.resolve(worktreePath)}`],
  });
...
export async function realizeExecutionWorkspace(input: {
  ...
  async function reuseExistingWorktree(reusablePath: string, effectiveBranchName = branchName, extraWarnings: string[] = []) {
    let refresh: { refreshed: boolean; baseRefSha: string | null };
    if (reuseSyncStrategy && currentBaseRefSha && !requestedExistingBranch) {
      ...
      refresh = { refreshed: true, baseRefSha: currentBaseRefSha };
    } else {
      refresh = currentBaseRefSha && !requestedExistingBranch
...
export async function allocateRuntimeServicePort(overrides?: {
  ...
  for (let attempt = 0; attempt < PORT_ALLOCATION_ATTEMPTS; attempt += 1) {
    ...
    // The broker owns both dedicated managed-HTTPS ranges. A generic auto-port
    // runtime must not become an untracked occupant that startup reconciliation
    // can only classify as exposure reservation drift.
    if (isAllowedRuntimeExposurePort(candidate)) continue;
