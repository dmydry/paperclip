...
export function decisionService(db: Db, options: DecisionServiceOptions) {
  ...
  async function snapshots(companyId: string, ids: string[], actor: AuthorizationActor, requiredActions: ReturnType<typeof targetActions>,
    ...
    // A proposal does not execute the effect directly: it records a bounded
    // action for a later board decision. Evaluate the agent's durable delegated
    // authority here, while keeping run-scoped direct comments contained in the
    // ordinary issue-comment path.
    const authorityActor = actor.type === "agent" && actor.runId
      ? { ...actor, runId: undefined }
      : actor;
    const result: Record<string, Snapshot> = {};
    for (const issue of rows) {
      const access = await authz.decide({ actor: authorityActor, action: "issue:read", resource: resource(issue) });
      ...
      for (const action of requiredActions.get(issue.id) ?? []) {
        const effectAccess = await authz.decide({ actor: authorityActor, action, resource: resource(issue) });
      ...
      if (cancellationTarget && descendantIds.length) {
        ...
        const descendantAccess = await Promise.all(descendants.map((descendant) =>
          authz.decide({ actor: authorityActor, action: "issue:mutate", resource: resource(descendant) })));
  ...
  async function executeEffect(decision: typeof decisions.$inferSelect, effect: DecisionEffect, effectIndex: number,
    ...
    try {
      ...
      const executionResult = await db.transaction(async (tx) => {
        ...
        // Decision effects are approved and executed outside the originating run. Revalidate
        // the agent's durable delegated authority without applying direct-comment run scope.
        const originAuthorityActor: AuthorizationActor = { ...originActor, runId: undefined };
        const originAction = effect.type === "comment_on_issue" ? "issue:comment" : "issue:mutate";
        const originAccess = await Promise.all(referencedIssues.map((item) => authz.decide({ actor: originAuthorityActor, action: originAction, resource: resource(item) })));
