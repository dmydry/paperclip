...
- If you are woken with `wakeReason=issue_interaction_stale_target` or `workspaceRefreshReason=stale_request_confirmation_repair`, refresh the issue, latest `plan` document, and interactions; do not delegate or continue implementation. If approval is still needed, create one fresh `request_confirmation` bound to the latest plan revision with idempotency key `confirmation:{issueId}:plan:{revisionId}`; otherwise leave a concise blocker/comment explaining why no fresh approval is valid.
